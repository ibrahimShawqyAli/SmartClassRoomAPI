...
router.post("/", auth, requireAdmin, async (req, res) => {
  try {
    const { name, code, department_id, credit_hours } = req.body || {};
    if (!name) {
      return res.status(400).json({ status: false, error: "name is required" });
    }

    const sql = `
      INSERT INTO dbo.courses (name, code, department_id, credit_hours)
      OUTPUT INSERTED.id
      VALUES (@p0, @p1, @p2, @p3)
    `;
...
router.patch("/:id", auth, requireAdmin, async (req, res) => {
  try {
    const { id } = req.params;
    const { name, code, department_id, credit_hours } = req.body || {};
    if (!name && !code && !department_id && !credit_hours) {
      return res
        .status(400)
